ruleset pillage_no_village.user {
  ...
  global {
    __testing = { "queries":
      ...
      [ { "domain": "game", "type": "create", "attrs": [ "id", "token" ] }
      , { "domain": "user", "type": "update", "attrs": [ "email", "token" ] }
      ]
    }

    playerNumFromId = function(id) {
      nums = ["2", "4", "3"];
      nums[id - 1]
    }

    user = function() {
      ent:user.delete("salt").delete("password")
    }
    ...
    game = function() {
      sub = gameSub()
      sub => wrangler:skyQuery(sub{"Tx"}, "pillage_no_village.game", "game") | null
    }
    ...
    create_game = defaction(host, gameId, player_num) {
      every {
        event:send({"eci": wrangler:parent_eci(), "domain":"game", "type":"create", "attrs":{ "host": host, "id": gameId, "player_num": player_num }});
  ...
  rule update_user {
    ...
    pre {
      ...
      attrs = event:attrs.delete("salt").delete("password").delete("_headers").delete("token")
  ...
  rule create_game {
    ...
    pre {
      authenticated = checkToken(event:attrs{"token"})
      gameId = event:attrs{"id"}
      player_num = playerNumFromId(gameId)
      host = ent:user{"username"}
      directive = (authenticated) => "Authorized" | "Unauthorized"
    }
    choose directive {
      Authorized => create_game(host, gameId, player_num)
